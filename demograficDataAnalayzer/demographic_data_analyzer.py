...
def calculate_demographic_data(print_data=True):
    ...
    percentage_bachelors = ((df.loc[df['education'] == 'Bachelors'].shape[0] / df.education.count())*100).round(1)
    ...
    lower_education_rich = round((df.loc[((df['education-num'] < 13)|(df['education'] == 'Prof-school')) & (df['salary'] == '>50K')].shape[0]/ df.loc[(df['education-num'] < 13)| (df['education'] == 'Prof-school')].shape[0]*100),1)
    ...
    highest_earning_country_percentage = ((df.loc[df['salary'] == '>50K'].groupby('native-country')['native-country'].count()/df.groupby('native-country')['native-country'].count()).sort_values(ascending = False)[0]*100).round(1)
